...
def decrypt_key(session_key):
    ...
    rsa_key = RSA.importKey(open('thekeys').read())    

    cipher_rsa = PKCS1_OAEP.new(rsa_key)
    decrypt_key = cipher_rsa.decrypt(session_key)
    # print("Key successfuly decrypted", decrypt_key)
    return decrypt_key
...
def decrypt_message(client_message, session_key):
    ...
    decrypted = decryptor.decrypt(client_message)
    ...
    try:
        while True:
            ...
            try:
                ...
                msg = decrypt_message(ciphertext_message, plaintext_key)
                print(msg.decode())

                # TODO: Split response from user into the username and password
                user,psswd = splitmessage(msg)
                newmes = ""
                if verify_hash(user,psswd) == True:
                    newmes = "User Successfully Authenticated"
                else:
                    newmes = "Username or Password incorrect"
                # TODO: Encrypt response to client

                # Send encrypted response
                ciphertext_response = encrypt_message(newmes,plaintext_key)
